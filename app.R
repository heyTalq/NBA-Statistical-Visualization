...
arenaUrl = read_html("https://en.wikipedia.org/wiki/List_of_National_Basketball_Association_arenas")
nbaTable = data.frame(arenaUrl %>%
                        html_nodes(xpath = '//*[@id="mw-content-text"]/table[1]') %>%
                        html_table())
nbaTable[11,2] = "Sleep Train Arena" # change to old 2015 arena
...
abbUrl = read_html("https://en.wikipedia.org/wiki/Wikipedia:WikiProject_National_Basketball_Association/National_Basketball_Association_team_abbreviations")
  ...
  observeEvent(
    {input$teams
      input$seasons},{   # observe changes to team/season input
        ...
        csvFileNames =  paste0("./",input$seasons,"/",shortNames,".csv") # "Hawks.csv" , etc. in correct directories
        ...
        defData = read.csv(paste0("./",input$seasons,"/","default.csv"), colClasses = c("integer", "character", rep("numeric", 23)))
    ...
    {input$datateams
      ...
      input$dataseasons}, {
        ...
        if (input$datateams != "") {
          ...
          csvData = read.csv(paste0("./",input$dataseasons,"/",inputShortTeam,".csv"),
                             colClasses = c("integer", rep("character", 8), rep("integer",4), "character", "character"))
